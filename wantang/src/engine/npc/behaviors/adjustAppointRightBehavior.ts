...
import { executeToggleAppointRight, executeDeclareWar } from '@engine/interaction';
...
interface AdjustAppointRightData {
  vassalId: string;
  postId: string;
  territoryId: string;
  grant: boolean; // true=授予(讨好), false=收回(集权)
  tier: string;
}

export const adjustAppointRightBehavior: NpcBehavior<AdjustAppointRightData> = {
  id: 'adjustAppointRight',
  playerMode: 'skip',

  generateTask(actor: Character, ctx: NpcContext): BehaviorTaskResult<AdjustAppointRightData> | null {
    ...
    if (maxUrgency > APPEASE_THRESHOLD) {
      for (const t of targets) {
        ...
        for (const pp of posts) {
          if (pp.hasAppointRight) continue; // 已有
    ...
    if (minUrgency < CENTRALIZE_THRESHOLD) {
      for (let i = targets.length - 1; i >= 0; i--) {
        ...
        const posts = getVassalPolicyPosts(t.vassalId, ctx);
        for (const pp of posts) {
          if (!pp.hasAppointRight) continue; // 本来就没有
          if (!hasAuthorityOverPost(actor.id, pp.territoryId, ctx.territories)) continue;
  ...
  executeAsNpc(actor: Character, data: AdjustAppointRightData, ctx: NpcContext) {
...
registerBehavior(adjustAppointRightBehavior);
